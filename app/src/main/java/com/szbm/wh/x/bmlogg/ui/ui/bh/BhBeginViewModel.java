...
public class BhBeginViewModel extends ViewModel {
   ...
   public final LiveData<Bh_extra_info> extra_infoLiveData = Transformations.switchMap(boreholeSetLiveData,bh->
    ...
    public final LiveData<Bh_begin_info> server_result = Transformations.switchMap(boreholeSetLiveData, bhset->{
        if(bhset != null){
            return  extraRepository.loadbh_begin_infoFromDisk(bhset,bh_logger.getNumber());
        }
        else{
    ...
    public final LiveData<List<Bh_imagesinfo>> server_imageInfos = Transformations.switchMap(server_result, bh_begin_info->{
        if(bh_begin_info != null){
            return  extraRepository.getImageinfos(bh_begin_info.getIid(),"Bh_begin_info");
        }
        else{
            return AbsentLiveData.Companion.create();
        }
    ...
    public final LiveData<Bh_geo_date> server_geo_date = Transformations.switchMap(server_result, bh_begin_info->{
        if(bh_begin_info != null){
            return  extraRepository.getbh_geo_date(bh_begin_info.getIid(),"Bh_begin_info");
        }
        else{
            return AbsentLiveData.Companion.create();
        }
    ...
    public LiveData<Bh_imagesinfo> addImageResult = Transformations.switchMap(addUri,uri->{
        if(uri == null){
            ...
            Bh_begin_info bh_begin_info = server_result.getValue();
            Bh_imagesinfo bh_imagesinfo = new Bh_imagesinfo(null,
                    "Bh_begin_info",
                    bh_begin_info.getIid(),
                    null,
                    uri.toString(),
                    bh_begin_info.getBorehole_extra_iid());
