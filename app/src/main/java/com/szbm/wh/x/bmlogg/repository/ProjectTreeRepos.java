...
//    }
//
    public LiveData<Resource<MainProjectInfo>> loadProject(long projectid){
        MediatorLiveData<Resource<MainProjectInfo>> result = new MediatorLiveData<>();
        result.setValue(Resource.Companion.loading(null));
        bmloggExecutors.diskIO().execute(()->{
            ProjectInfo projectInfo = bmLoggDb.projectInfoDao().selectEntity(projectid);
            int count =bmLoggDb.bh_BoreholeInfoDao().count(projectid);
            bmloggExecutors.mainThread().execute(()->{
                if(projectInfo == null)
                    result.setValue(Resource.Companion.error("加载当前项目失败！",null));
                else
                    result.setValue(Resource.Companion.success(
                            new MainProjectInfo(projectInfo,null,null,count)));
            });
        });
        return result;
    }
...
////        root.checked = projectInfo.getChecked()==1l;
